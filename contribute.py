...
def main(def_args=sys.argv[1:]):
    ...
    for day in (start_date + timedelta(n) for n in range(days_before + days_after + 1)):
        if (not no_weekends or day.weekday() < 5) and randint(5, 100) < frequency:
            for commit_time in (
    ...
    print("\nRepository updated " "\x1b[6;30;42msuccessfully\x1b[0m!")
...
def run(commands, silent=False):
    try:
        if silent:
            Popen(
                commands, stdout=open(os.devnull, "w"), stderr=open(os.devnull, "w")
            ).wait()
        else:
            Popen(commands).wait()
    except:
